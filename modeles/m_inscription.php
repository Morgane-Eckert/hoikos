...
function ajout_logement($telephone_fixe,$numero_rue_logement,$nom_rue_logement,$code_postale_logement,$ville_logement,$pays_logement){
	...
	$request = $bdd->query("SELECT numero_de_cemac FROM cemac");
	while ($verif = $request->fetch()){
		if ($verif[0] == $_POST["numero_de_cemac"]){
			$_SESSION["cemac_check"]=2;
			break;
		}
	}
	...
	if ($_SESSION["cemac_check"]==2){
		...
		$_SESSION["cemac_inscription"]=$_POST['numero_de_cemac'];
		return 2;
	}else{
		$_SESSION["cemac_check"]=1;
		return 1;
	}
}
