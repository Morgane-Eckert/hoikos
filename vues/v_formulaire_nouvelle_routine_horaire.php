							...
							<form method="post" action="index.php?target=compte&action=connecte&reaction=nouvelle_routine_horaire_rempli&comprehension=<?php echo $_GET['comprehension']; ?>&anticipation=<?php echo $_GET['anticipation']; ?>">
								...
								while ($donnees1 = $reponse1->fetch()) { 
									...
									<?php if($donnees1['jour_routine']=='Lundi'){ ?>
										<input type="checkbox" name="jours[]" value="Lundi" checked />Lundi
										<?php $jours[$donnees1['jour_routine']]=$donnees1['jour_routine']; ?>
									<?php }else{ ?>
										<?php if($donnees1['jour_routine']=='Mardi'){ ?>
											<input type="checkbox" name="jours[]" value="Mardi" checked />Mardi
											<?php $jours[$donnees1['jour_routine']]=$donnees1['jour_routine']; ?>
										<?php }else{ ?>
											<?php if($donnees1['jour_routine']=='Mercredi'){ ?>
												<input type="checkbox" name="jours[]" value="Mercredi" checked />Mercredi
												<?php $jours[$donnees1['jour_routine']]=$donnees1['jour_routine']; ?>
											<?php }else{ ?>
												<?php if($donnees1['jour_routine']=='Jeudi'){ ?>
													<input type="checkbox" name="jours[]" value="Jeudi" checked />Jeudi
													<?php $jours[$donnees1['jour_routine']]=$donnees1['jour_routine']; ?>
												<?php }else{ ?>
													<?php if($donnees1['jour_routine']=='Vendredi'){ ?>
														<input type="checkbox" name="jours[]" value="Vendredi" checked />Vendredi
														<?php $jours[$donnees1['jour_routine']]=$donnees1['jour_routine']; ?>
													<?php }else{ ?>
														<?php if($donnees1['jour_routine']=='Samedi'){ ?>
															<input type="checkbox" name="jours[]" value="Samedi" checked />Samedi
															<?php $jours[$donnees1['jour_routine']]=$donnees1['jour_routine']; ?>
														<?php }else{ ?>
															<?php if($donnees1['jour_routine']=='Dimanche'){ ?>
																<input type="checkbox" name="jours[]" value="Dimanche" checked />Dimanche
																<?php $jours[$donnees1['jour_routine']]=$donnees1['jour_routine']; ?>
															<?php } ?>
									<?php  } } } } } }?>	
									<br />
								...
								?>
								<br><label for="type">Sélectionner une heure de début :</label><br>
